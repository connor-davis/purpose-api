...
const generateAllUsersExcelSheet = async (
  usersData,
  productsData,
  produceData,
  harvestsData,
  ...
  try {
    ...
    let userProfilesSheet = workbook.addWorksheet('Profiles', {
      headerFooter: { firstHeader: 'Profiles' },
    });
    ...
    usersData.forEach((userData) => {
      if (userData.businessType === 'admin') return;
    ...
    let userProduceSheet = workbook.addWorksheet('Produce', {
    ...
    produceData.forEach((d) => {
      ...
      userProduceSheet.addImage(
        imageId,
        `A${imageRowProduce}:A${imageRowProduce}`
      );
    ...
    workbook.xlsx
      .writeFile(path.join(process.cwd(), 'temp', 'all-users-data.xlsx'))
      .then(() => {
        callback(path.join(process.cwd(), 'temp', 'all-users-data.xlsx'));
      });
  } catch (error) {
    console.log(error);
  }
...
module.exports = generateAllUsersExcelSheet;
